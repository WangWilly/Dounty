...
pub mod bounty_factory {
    ...
    pub fn create_v1(ctx: Context<CreateV1Acc>, title: String, url: String, commissioners: Vec<Pubkey>, asignee: Option<Pubkey>) -> Result<()> {
        if title.len() > 50 {
    ...
    pub fn update_v1(ctx: Context<UpdateV1Acc>, title: Option<String>, url: Option<String>, commissioners: Option<Vec<Pubkey>>, asignee: Option<Pubkey>) -> Result<()> {
        let bounty = &mut ctx.accounts.bounty;
    ...
    pub fn issue_v1(ctx: Context<IssueV1Acc>) -> Result<()> {
        if ctx.accounts.bounty.asignee.is_none() {
            return Err(ErrorCode::BountyNotAssigned.into());
        }
        if ctx.accounts.bounty.asignee.unwrap() != *ctx.accounts.assignee.key {
            return Err(ErrorCode::WrongAssignee.into());
        }

        let commissioner_num = ctx.accounts.bounty.commissioners.len();
        ...
        let comminssioners: HashSet<Pubkey> = ctx.accounts.bounty.commissioners.iter().cloned().collect();
        ...
        let bounty = **ctx.accounts.bounty.to_account_info().try_borrow_lamports()?;
        let from_account = ctx.accounts.bounty.to_account_info();
        let to_account = ctx.accounts.assignee.clone();
        let transfer_instruction = system_instruction::transfer(from_account.key, to_account.key, bounty);
    ...
    pub fn create_doner_v1(ctx: Context<CreateDonerV1Acc>, donation: u64, message: String) -> Result<()> {
        if message.len() > 280 {
            return Err(ErrorCode::StringTooLong.into());
        }
        ...
        let transfer_instruction = system_instruction::transfer(from_account.key, to_account.key, donation);
...
pub struct CreateDonerV1Acc<'info> {
    ...
    #[account(init, payer = doner, space = DonerV1::INIT_SPACE, seeds = [b"doner", bounty.key().as_ref()], bump)]
