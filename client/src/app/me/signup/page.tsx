...
import { createAccount } from "@/clients/userClient/functions";
...
function SignUpPage() {
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const hashedPassword = await safe(hashPassword(password));
    if (!hashedPassword.success) {
      console.error(hashedPassword.error);
      return;
    }

    const sessionCreateRes = await safe(createAccount({ email, password: hashedPassword.data }));
  ...
  return (
    <div className="font-[sans-serif] bg-white max-w-4xl flex items-center mx-auto md:h-screen p-4">
      <div className="grid md:grid-cols-3 items-center shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] rounded-xl overflow-hidden">
        ...
        <form className="md:col-span-2 w-full py-6 px-6 sm:px-16 max-md:max-w-xl mx-auto" onSubmit={handleSignup}>
          <div className="mb-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="relative flex items-center">
                <input name="email" type="email" required className="text-gray-800 bg-white border border-gray-300 w-full text-sm pl-4 pr-8 py-2.5 rounded-md outline-blue-500" placeholder="Enter email" onChange={(e) => setEmail(e.target.value)} />
            ...
            <div>
              ...
              <div className="relative flex items-center">
                <input name="password" type="password" required className="text-gray-800 bg-white border border-gray-300 w-full text-sm pl-4 pr-8 py-2.5 rounded-md outline-blue-500" placeholder="Enter password" onChange={(e) => setPassword(e.target.value)} />
          ...
          <div className="mt-8">
            <Button type="submit" className="w-full py-2.5 px-4 tracking-wider text-sm rounded-md text-white bg-gray-700 hover:bg-gray-800 focus:outline-none">
              Create an account
            </Button>
          </div>
